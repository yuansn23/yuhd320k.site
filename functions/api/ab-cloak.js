  ...
  try {
    ...
    if (t.username) {
      try {
        ...
        await env.DB.prepare('INSERT INTO stats_daily (username, date, ab_total, ab_pass) VALUES (?1, ?2, 1, ?3) ON CONFLICT(username, date) DO UPDATE SET ab_total = ab_total + 1, ab_pass = ab_pass + ?3')
