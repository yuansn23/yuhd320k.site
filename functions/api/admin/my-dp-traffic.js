// GET /api/admin/my-dp-traffic — 子账户 DP流量 访问列表（独立表 dp_traffic）
...
export async function onRequest(context) {
  ...
  try {
    ...
    // 总数（走 idx_dp_traffic_user 索引 COUNT）
    var total = 0;
    try {
      var cs = 'SELECT COUNT(*) AS cnt FROM dp_traffic WHERE ' + where;
      var cnt = null;
    ...
    var rows = [];
    try {
      var ds = 'SELECT site, ip, device, lang, timezone, is_vpn, is_proxy, passed, redirect_to, triggered_rules, created_at FROM dp_traffic WHERE ' + where + ' ORDER BY created_at DESC LIMIT ?' + (idx++) + ' OFFSET ?' + (idx++);
      ...
      if (res && res.results) rows = res.results.map(function (r) {
        ...
        return {
          site: r.site, ip: r.ip, device: r.device, lang: r.lang, timezone: r.timezone,
