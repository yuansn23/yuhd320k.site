...
export async function onRequest(context) {
  ...
  try {
    ...
    if (dateStart && dateEnd) {
      ...
      result = await env.DB.prepare('SELECT site, visit_time, ip, device, lang FROM visit_logs WHERE username = ?1 AND visit_time >= ?2 ORDER BY visit_time DESC LIMIT ?3')
        .bind(me.user, dateStart + 'T00:00:00.000Z', limit).all();
    } else if (filterSite) {
      result = await env.DB.prepare('SELECT site, visit_time, ip, device, lang FROM visit_logs WHERE username = ?1 AND site = ?2 ORDER BY visit_time DESC LIMIT ?3')
        .bind(me.user, filterSite, limit).all();
    } else {
      result = await env.DB.prepare('SELECT site, visit_time, ip, device, lang FROM visit_logs WHERE username = ?1 ORDER BY visit_time DESC LIMIT ?2')
