...
export async function onRequest(context) {
  ...
  try {
    ...
    // 总数（无 site/date 筛选时读预聚合表 stats_daily，result 在聚合上算；有筛选时走索引 COUNT）
    var total = 0;
    var useAgg = !filterSite && !dateStart && !dateEnd;
    if (useAgg) {
      try {
        var aggExpr;
        if (result === 'pass' || result === '1') aggExpr = 'COALESCE(SUM(cloak_pass),0)';
        else if (result === 'fail' || result === '0') aggExpr = 'COALESCE(SUM(cloak_total)-SUM(cloak_pass),0)';
        else aggExpr = 'COALESCE(SUM(cloak_total),0)';
