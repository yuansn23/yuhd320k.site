// GET/POST /api/admin/dp-perm — 管理员管理子账户的 DP配置 权限
// 独立于 accounts.js，不改动任何既有接口。权限存独立表 dp_permissions（默认关闭）。
  ...
  try {
    ...
    if (request.method === 'GET') {
      ...
      try {
        ...
        var permMap = {};
        try {
          var pr = await env.DB.prepare('SELECT username, enabled FROM dp_permissions').all();
          if (pr && pr.results) {
        ...
        if (users && users.results) {
          list = users.results.map(function (u) {
            return { username: u.username, site: u.site || '', dp_enabled: permMap[u.username] || 0 };
          });
        }
      } catch (e) {}
    ...
    if (request.method === 'POST') {
      ...
      var cur = 0;
      try { var cr = await env.DB.prepare('SELECT enabled FROM dp_permissions WHERE username = ?1').bind(username).first(); cur = (cr && cr.enabled === 1) ? 1 : 0; } catch (e) {}
      var next = cur ? 0 : 1;
      await env.DB.prepare('INSERT INTO dp_permissions (username, enabled) VALUES (?1, ?2) ON CONFLICT(username) DO UPDATE SET enabled = excluded.enabled').bind(username, next).run();
      return new Response(JSON.stringify({ ok: true, username: username, dp_enabled: next, msg: next ? '已开启DP配置权限' : '已关闭DP配置权限' }), {
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
      });
    }
